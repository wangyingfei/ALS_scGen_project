...
def reconstruct():
    ...
    for idx, cell_type in enumerate(data.obs["cell_type"].unique().tolist()):
        ...
        print(cell_type)
    #YW: added the two lines above sc.write to create unexisted folder, otherwise will fail to save
    filename = filename = f"../data/reconstructed/PCAVecArithm/PCA_pbmc.h5ad"
...
def train(data_name="pbmc", cell_type="CD4T", p_type="unbiased"):
    ...
    #YW: to handle sparse matrix problem, add a new variable "..._dense" and use this variable to process, same kind of changes in following codes
    if sparse.issparse(train_real_cd.X):
        train_real_cd_dense = train_real_cd.X.A
        train_real_stimulated_dense = train_real_stimulated.X.A
    else:
        train_real_cd_dense = train_real_cd.X
        train_real_stimulated_dense = train_real_stimulated.X
    train_real_stimulated_PCA = pca.transform(train_real_stimulated_dense)
    train_real_cd_PCA = pca.transform(train_real_cd_dense)
    ...
    if sparse.issparse(adata_list[1].X):
         adata_list_1_dense = adata_list[1].X.A
         adata_list_2_dense = adata_list[2].X.A
    else:
         adata_list_1_dense = adata_list[1].X
         adata_list_2_dense = adata_list[2].X
    ctrl_CD4T_PCA = pca.transform(adata_list_1_dense)
    predicted_cells = predict(pca, train_real_cd_PCA,train_real_stimulated_PCA,ctrl_CD4T_PCA, p_type)
    all_Data = sc.AnnData(np.concatenate([adata_list_1_dense, adata_list_2_dense, predicted_cells]))
    all_Data.obs["condition"] = ["ctrl"]*len(adata_list_1_dense) + ["real_stim"]*len(adata_list_2_dense) + ["pred_stim"]*len(predicted_cells)
    all_Data.var_names = adata_list[3].var_names
    # modification finished

    if p_type == "unbiased":
        filename = f"../data/reconstructed/PCAVecArithm/PCA_CD4T.h5ad"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        sc.write(f"../data/reconstructed/PCAVecArithm/PCA_CD4T.h5ad",all_Data)
    else:
        filename = f"../data/reconstructed/PCAVecArithm/PCA_CD4T_biased.h5ad"
        os.makedirs(os.path.dirname(filename), exist_ok=True)
